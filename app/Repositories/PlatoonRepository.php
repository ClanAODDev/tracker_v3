...
class PlatoonRepository
{
     ...
     */
    public function getPlatoonForumActivity(\App\Models\Platoon $platoon)
    {
        ...
        $oneMonth = $platoon->members()->where('last_activity', '<=', $twoWeeksAgo)->where('last_activity', '>=', $oneMonthAgo)->count();
    ...
    public function getPlatoonTSActivity(\App\Models\Platoon $platoon)
    {
        $twoWeeksAgo = \Carbon\Carbon::now()->subDays(14);
        $oneMonthAgo = \Carbon\Carbon::now()->subDays(30);
        $twoWeeks = $platoon->members()->where('last_ts_activity', '>=', $twoWeeksAgo)->count();
        $oneMonth = $platoon->members()->where('last_ts_activity', '<=', $twoWeeksAgo)->where('last_ts_activity', '>=', $oneMonthAgo)->count();
