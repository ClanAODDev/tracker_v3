...
class Member extends \Illuminate\Database\Eloquent\Model
{
     ...
     * @var array
     */
    protected $fillable = ['name', 'clan_id', 'platoon_id', 'squad_id', 'position_id', 'division_id', 'posts', 'join_date', 'last_activity', 'last_ts_activity', 'last_promoted_at', 'recruiter_id'];
    /**
     * @var array
     */
    protected $dates = [
    ...
    public function assignPosition($position)
    {
        ...
        if (in_array($newPosition->name, ["Commanding Officer", "Executive Officer", "General Sergeant", "Clan Admin"])) {
            $this->platoon_id = 0;
            $this->squad_id = 0;
        }
    ...
    {
        $this->update(['division_id' => 0, 'platoon_id' => 0, 'squad_id' => 0, 'position_id' => 1, 'flagged_for_inactivity' => false]);
    }
    /**
     * Handle Staff Sergeant assignments
     * division/
