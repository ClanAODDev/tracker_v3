	...
	public function __construct($user, $member) {
		...
		if ($user->role > 1) {
			...
			if (LeaveOfAbsence::count_expired($member->game_id) > 0 || LeaveOfAbsence::count_pending($member->game_id) > 0) {
				array_push($this->all, "<div class='alert alert-warning'><i class='fa fa-clock-o'></i> There are leaves of absence that require your attention <a href='manage/leaves-of-absence' class='alert-link'>Manage leaves of absence</a></div>");
			}
		}
