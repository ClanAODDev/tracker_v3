...
class Division extends Application {
	...
	public static function findDivisionLeaders($gid) {
		$sql = "SELECT member.member_id, member.forum_name, rank.abbr as rank, member.battlelog_name, position.desc as position_desc FROM member LEFT JOIN rank on member.rank_id = rank.id LEFT JOIN `position` ON member.position_id = position.id WHERE position_id IN (1,2) AND member.game_id = {$gid}";
		$params = Flight::aod()->sql($sql)->many();
		return arrayToObject($params);
	}

	public static function findGeneralSergeants($gid) {
		$sql = "SELECT member.member_id as forum_id, member.forum_name, rank.abbr as rank, position.desc as position_desc, member.battlelog_name FROM member LEFT JOIN rank on member.rank_id = rank.id LEFT JOIN `position` ON member.position_id = position.id WHERE position_id = 3 AND member.game_id = {$gid} AND status_id = 1";
		$params = Flight::aod()->sql($sql)->many();
		return arrayToObject($params);
	}

	public static function findSquadLeaders($gid, $order_by_rank = false) {
		$sql = "SELECT last_activity, rank.abbr, member_id, forum_name, platoon.name, member.battlelog_name FROM member LEFT JOIN platoon ON platoon.id = member.platoon_id LEFT JOIN rank ON rank.id = member.rank_id WHERE member.game_id = {$gid} AND position_id = 5";
