@if ($count = $division->memberRequests()->cancelled()->count())
    <div class="alert alert-warning">
        You have <code>{{ $count }}</code> {{ Str::plural('member request', $count) }} in need of attention. <a class="alert-link pull-right"
...
@if($division->outstandingInactives)
    <div class="alert alert-default">
        You have
        <code>{{ $division->outstandingInactives }}</code> outstanding inactive {{ Str::plural('member', $division->outstandingInactives) }}. AOD does not allow divisions to maintain members whose last forum activity exceeds
...
@if (count($division->mismatchedTSMembers))
    <div class="alert alert-default">
        You have
        <code>{{ count($division->mismatchedTSMembers) }}</code> {{ Str::plural('member', count($division->mismatchedTSMembers)) }} improperly configured for Teamspeak. Please review the
...
@if (count($division->unassigned) > 0)
    ...
    @can('create', [App\Platoon::class, $division])
        <div class="alert alert-default">
            ...
            You have
            <code>{{ count($division->unassigned) }}</code> unassigned {{ Str::plural('member', count($division->unassigned)) }} in
