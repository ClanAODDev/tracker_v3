...
<div class='page-header'>
	<h2>
		<strong><img src='/images/game_icons/48x48/{{ $division->abbreviation }}.png' /> {{ $division->name }} Division</strong>
			...
			<div class='list-group'>
				...
				@if (count($division->platoons))
					@foreach ($division->platoons as $platoon)
						<a href='divisions/{{ $division->abbreviation }}/platoons/{{ $platoon->id }}' class='list-group-item'>
			...
			@if (count($division->leaders))
				@foreach ($division->leaders as $leader)
					<a href='members/{{ $leader->clan_id }}' class='list-group-item'>
						...
						<h4 class='list-group-item-heading'><strong>{{ $leader->rank->abbreviation }} {{ $leader->name }}</strong></h4>
						<p class='list-group-item-text text-muted'><?php echo $leader->position->name; ?></p>
					</a>
				@endforeach
			@else
				<li class="list-group-item">This division has no assigned leadership</li>
			@endif
		</div>
	</div>
</div>
