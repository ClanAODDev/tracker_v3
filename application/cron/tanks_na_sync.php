...
if (dbConnect()) {
	...
	try {
		...
		$params = $pdo->query("SELECT m.member_id, h.handle_value FROM member m INNER JOIN member_handles h ON h.member_id = m.id WHERE m.id = {$next_player} AND m.status_id = 1 AND m.game_id = 3 AND h.handle_type = 7")->fetch(); 
