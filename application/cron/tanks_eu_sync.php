...
if (dbConnect()) {
	...
	try {
		...
		$limit = $pdo->query("SELECT max(member_id) as max, min(member_id) as min FROM member_handles WHERE handle_type = 8")->fetch();
		...
		if (empty($params)) {
			...
			$profile = download_tanks_profile($params['handle_value'], 8);
